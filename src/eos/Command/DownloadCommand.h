...
public:
	DownloadCommand(CameraModel *model, EdsDirectoryItemRef dirItem)
			: Command(model), _directoryItem(dirItem){}
	...
	virtual bool execute()
	{
        ...
        if(err == EDS_ERR_OK)
		{
            if(strstr(dirItemInfo.szFileName, ".THM") != 0)
            {
                EdsDownloadCancel(_directoryItem);
                EdsRelease(_directoryItem);
                return true;
            }
		}
		...
		if(err == EDS_ERR_OK)
		{
			//err = EdsCreateFileStream(dirItemInfo.szFileName, kEdsFileCreateDisposition_CreateAlways, kEdsAccess_ReadWrite, &stream);
             err = EdsCreateMemoryStream(20480L, &stream);
		}
        ...
        if (err == EDS_ERR_OK)
        {
            CameraEvent e("ImageData", &stream);
            _model->notifyObservers(&e);
        }

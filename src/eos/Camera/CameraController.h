...
#include "ExtendShutDownTimerCommand.h"
...
public:
	...
	void actionPerformed(const ActionEvent& event)
	{
		...
		if ( command == "set_WhiteBalance" )
		{
			StoreAsync(new SetPropertyCommand<EdsUInt32>(_model, kEdsPropID_WhiteBalance, *static_cast<EdsUInt32*>(event.getArg())));
		}
		...
		if ( command == "ExtendShutDownTimer" )
		{
			StoreAsync(new ExtendShutDownTimerCommand(_model));
		}
